export class AltCard {
  ...
  static createCards() {
    ...
    for(let suit in AltCard.suits) {
      let currentSuit = AltCard.suits[suit];
      for(let rank in AltCard.ranks) {
        let currentRank = AltCard.ranks[rank];
        let currentCard = new AltCard(currentSuit,  currentRank);
  ...
  static getSuitChoices() {
    return Object.keys(AltCard.suits);
  }

  static getRankChoices() {
    return Object.keys(AltCard.ranks);
  }
